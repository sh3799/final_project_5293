...
cuisine.top10 <- res %>% group_by(`CUISINE DESCRIPTION`) %>% tally() %>% top_n(10)
top10.type <- cuisine.top10$`CUISINE DESCRIPTION` 
res_gcuisine <- res %>% filter(GRADE==c("A","B","C"))%>% filter(`CUISINE DESCRIPTION`==top10.type) %>% group_by(`CUISINE DESCRIPTION`,GRADE) %>% tally() 
...
```{r}
res_cuisine10 <- res_one %>%  filter(`CUISINE DESCRIPTION`==top10.type) %>% select(`CUISINE DESCRIPTION`,SCORE,GRADE,CAMIS,DBA)
#colnames(res_cuisine10) <- c("Cuisine","Score")
library("ggridges")
par(mfrow=c(2,1))
ggplot(res_cuisine10,aes(x=SCORE, y=`CUISINE DESCRIPTION`, fill=`CUISINE DESCRIPTION`))+
  geom_density_ridges(scale=2,alpha=0.5)+xlim(0,35)+
  labs(x="Scores deducted", y="Cuisine Types")+
  ggtitle("")

ggplot(res_cuisine10,aes(x=SCORE, y=`CUISINE DESCRIPTION`, fill=GRADE))+
...
vis_miss(res_raw,warn_large_data = FALSE)
library(naniar)
gg_miss_var(res_raw)
```