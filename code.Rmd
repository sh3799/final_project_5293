...
res_unique <-
  read_csv("restaurant_data.csv")[, 1:8] %>% distinct() %>% filter(BORO != "Missing")
...
res_borough_t <- res_borough %>% group_by(`BORO`, `CUISINE DESCRIPTION`) %>% mutate(count = n()) %>% distinct() %>% arrange(desc(count)) %>% group_by(BORO) %>% top_n(10)

ggplot(res_borough_t, aes(
  x = count,
...
res_borough_nonA <- filter(res_borough_t,`CUISINE DESCRIPTION`!=c("American","Other"))
ggplot(res_borough_nonA, aes(
  x = count,
  y = fct_reorder2(`CUISINE DESCRIPTION`, BORO =="MANHATTAN", -count),
  color = BORO
)) +
...
ggplot(res_borough_nonA, aes(
  ...
  geom_bar(position="dodge", stat="identity") + ylab("")+facet_wrap(~`CUISINE DESCRIPTION`) + theme(axis.title.x=element_blank(),
...
res_columbia_sum <-
  res_columbia %>% group_by(`CUISINE DESCRIPTION`) %>% summarize(total =n()) %>% arrange(desc(total))%>% top_n(20)
# histogram
...
```{r}
res_cuisine10 <-
  res_one %>%  filter(`CUISINE DESCRIPTION` == top10.type) %>% select(`CUISINE DESCRIPTION`, SCORE, GRADE, CAMIS, DBA)
library("ggridges")
par(mfrow = c(2, 1))
ggplot(res_cuisine10,
...
datematch <- res$`INSPECTION DATE`==res$`GRADE DATE`
mean(datematch)

res_na <- res_raw %>% filter(is.na(GRADE))%>%
  group_by(`Month` = floor_date(`INSPECTION DATE`, "month")) %>%tally()
